...
RC unpinPage(BM_BufferPool *const bufferPool, BM_PageHandle *const pageHandle)
{
    ...
    BM_BufferPool *bufferManager; //TODO old code:BM_BufferPool_Mgmt *bufferManager;
    ...
    BM_PageHandle *currentFrame = bufferManager->head;//TODO old code: PageFrame *currentFrame = bufferManager->head;
...
RC forcePage(BM_BufferPool *const bufferPool, BM_PageHandle *const pageHandle)
{
    ...
    do
    {
        ...
        if (currentFrame->pageNum == pageHandle->pageNum && currentFrame->dirtyFlag == 1)
        {
            ...
            currentFrame->dirtyFlag = 0;

            break; // Exit the loop once the required page has been processed
        }

        currentFrame = currentFrame->next; // Move to the next frame in the circular linked list
    } while (currentFrame != bufferManager->head); // Stop looping once we return to the starting frame
...
RC pinPageFIFO(BM_BufferPool *const bufferPool, BM_PageHandle *const pageHandle, const PageNumber pageNumber)
{
    ...
    BM_BufferPool *bufferControl= bufferPool->mgmtData;
    //TODO old code//BM_BufferPool_Mgmt *bufferControl = bufferPool->mgmtData;

    // Pointer to the first frame in the buffer pool
    //TODO old code: //PageFrame *currentFrame = bufferControl->head;
    BM_PageHandle *currentFrame = bufferControl->head;
...
RC pinPageCLOCK(BM_BufferPool *const bufferPool, BM_PageHandle *const pageHandle, const PageNumber pageNumber)
{
    ...
    // Access the buffer pool's internal management data
    BM_BufferPool *bufferManager= bufferPool->mgmtData;
    //TODO old code//BM_BufferPool_Mgmt *bufferManager = bufferPool->mgmtData;

    // Pointer to the first frame in the buffer pool
    //TODO old code: //PageFrame *currentFrame = bufferManager->head;
    BM_PageHandle *currentFrame = bufferManager->head;
    ...
    BM_PageHandle *replacementFrame;//TODO changed code from pageframe to BM
...
PageNumber *getFrameContents(BM_BufferPool *const bufferPool)
{
    ...
    BM_BufferPool *bufferManager;//BM_BufferPool_Mgmt *bufferManager;
    ...
    BM_PageHandle *framePtr = bufferManager->start;//todo PageFrame *framePtr = bufferManager->start;
